...
} from '@/redux/features/admin/products/categoryManagement.api';
...
export default function List() {
  ...
  const { data: apiResponse, isFetching } = useGetAllCategoriesQuery(params);
  const [updateCategory] = useUpdateCategoryMutation();

  const records: ICategory[] = apiResponse?.data || [];
  const meta = apiResponse?.meta || { page: 1, limit: rowsPerPage, total: 0, totalPage: 1 };
  const filterFn = { fn: (items: ICategory[]) => items };

  const { TblPaginationSimple, recordsAfterPagingAndSorting } = useTable<ICategory>({
    records,
    headCells,
    filterFn,
    meta,
  ...
  const handleStatusUpdate = async (item: ICategory, newStatus: boolean) => {
  ...
  const updateStatus = (e: React.ChangeEvent<HTMLInputElement>, item: ICategory) => {
  ...
  return (
    <>
      <Paper sx={{ overflow: 'hidden', p: 2, width: '100%' }}>
        ...
        >
          <PageHeader title="Categories" subTitle="A list of all Categories" />
          <Controls.Button
            text="Add Category"
            component={Link}
            to={`/${currentRole.toLowerCase()}/category`}
              ...
              {recordsAfterPagingAndSorting().length === 0 ? (
                ...
                recordsAfterPagingAndSorting().map((item, index) => (
                  <TableRow key={item._id}>
                    ...
                    <TableCell>
                      <Link
                        to={`/${currentRole.toLowerCase()}/category/${item._id}`}
