...
const headCells = [
  { id: 'number', label: '#' },
  { id: 'attributeGroupName', label: 'Group' },
  { id: 'name', label: 'Name' },
...
export default function AttributeValues() {
  ...
  const { data: apiResponse, isFetching } = useGetAllAttributeValuesQuery(params);
  const records = apiResponse?.data || [];
  const meta = apiResponse?.meta || { page: 1, limit: rowsPerPage, total: 0, totalPage: 1 };
  const filterFn = { fn: (items: IAttributeValue[]) => items };
  ...
  const handleStatusUpdate = async (item: IAttributeValue, newStatus: boolean) => {
    setConfirmDialog({ isOpen: false, title: '', subTitle: '', onConfirm: () => { } });

    const updatedItem: Partial<IAttributeValue> = { isActive: newStatus };
    const toastId = toast.loading('Updating...', { position: 'top-right' });

    try {
      await updateAttributeValue({ id: item._id, requestData: updatedItem }).unwrap();
  ...
  const updateStatus = (e: React.ChangeEvent<HTMLInputElement>, item: IAttributeValue) => {
  ...
  return (
    <>
      <Paper sx={{ overflow: 'hidden', p: 2 }}>
        ...
        >
          <PageHeader title="Attribute Values" subTitle="Manage attribute values" />
          <Controls.Button
            text="Add Attribute Value"
            component={Link}
            to={`/${currentRole.toLowerCase()}/attribute-value`}
              ...
              {recordsAfterPagingAndSorting().length === 0 ? (
                ...
                recordsAfterPagingAndSorting().map((item, index) => (
                  <TableRow key={item._id}>
                    ...
                    <TableCell>{item.attributeGroupName || '-'}</TableCell>
                    ...
                    <TableCell>
                      <Link to={`/${currentRole.toLowerCase()}/attribute-value/${item._id}`} style={linkStyle}>
                        Update
                      </Link>
                    </TableCell>
                  </TableRow>
