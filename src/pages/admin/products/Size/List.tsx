...
} from '@/redux/features/admin/products/sizeManagement.api';
import { useAppSelector } from '@/redux/hooks';
import { selectCurrentRole } from '@/redux/features/auth/authSlice';
import { ISize } from '@/types/products.type';
...
const SizeList = () => {
  ...
  const { data: apiResponse, isFetching } = useGetAllSizeQuery(params);
  const [updateSize] = useUpdateSizeMutation();

  const records: ISize[] = apiResponse?.data || [];
  const meta = apiResponse?.meta || { page: 1, limit: rowsPerPage, total: 0, totalPage: 1 };
  const filterFn = { fn: (items: ISize[]) => items };

  const { TblPaginationSimple, recordsAfterPagingAndSorting } = useTable<ISize>({
    records,
    headCells,
    filterFn,
    meta,
  ...
  const handleStatusUpdate = async (item: ISize, newStatus: boolean) => {
    setConfirmDialog({ isOpen: false, title: '', subTitle: '', onConfirm: () => {} });
    const toastId = toast.loading('Updating...', { position: 'top-right' });

    try {
      await updateSize({ id: item._id, requestData: { ...item, isActive: newStatus } }).unwrap();
      toast.success('Size status updated successfully', { id: toastId, position: 'top-right' });
    } catch (error) {
      toast.error('Failed to update Size status', { id: toastId, position: 'top-right' });
      console.error(error);
    }
  };

  const updateStatus = (e: React.ChangeEvent<HTMLInputElement>, item: ISize) => {
  ...
  return (
    <>
      <Paper sx={{ overflow: 'hidden', p: 2, width: '100%' }}>
        ...
        >
          <PageHeader title="Sizes" subTitle="A list of all Sizes" />
          <Controls.Button
            text="Add Size"
            component={Link}
            to={`/${currentRole.toLowerCase()}/Size`}
              ...
              {recordsAfterPagingAndSorting().length === 0 ? (
                ...
                recordsAfterPagingAndSorting().map((item, index) => (
                  <TableRow key={item._id}>
                    ...
                    <TableCell>
                      <Link
                        to={`/${currentRole.toLowerCase()}/size/${item._id}`}
