...
const headCells = [
  { id: 'number', label: '#' },
  { id: 'name', label: 'Name' },
  { id: 'measurementUnitSymbol', label: 'Symbol' },
...
export default function Measurements() {
  ...
  const { data: apiResponse, isFetching } = useGetAllMeasurementsQuery(params);
  const records = apiResponse?.data || [];
  const meta = apiResponse?.meta || { page: 1, limit: rowsPerPage, total: 0, totalPage: 1 };
  const filterFn = { fn: (items: IMeasurementUnit[]) => items };
  ...
  const handleStatusUpdate = async (item: IMeasurementUnit, newStatus: boolean) => {
    setConfirmDialog({ isOpen: false, title: '', subTitle: '', onConfirm: () => { } });

    const updatedItem: Partial<IMeasurementUnit> = { isActive: newStatus };
    const toastId = toast.loading('Updating...', { position: 'top-right' });

    try {
      await updateMeasurement({ id: item._id, requestData: updatedItem }).unwrap();
  ...
  const updateStatus = (e: React.ChangeEvent<HTMLInputElement>, item: IMeasurementUnit) => {
  ...
  return (
    <>
      <Paper sx={{ overflow: 'hidden', p: 2 }}>
        ...
        >
          <PageHeader title="Measurement Units" subTitle="Manage measurement units" />
          <Controls.Button
            text="Add Measurement"
            component={Link}
            to={`/${currentRole.toLowerCase()}/measurement`}
              ...
              {recordsAfterPagingAndSorting().length === 0 ? (
                ...
                recordsAfterPagingAndSorting().map((item, index) => (
                  <TableRow key={item._id}>
                    ...
                    <TableCell>{item.measurementUnitSymbol || '-'}</TableCell>
                    ...
                    <TableCell>
                      <Link to={`/${currentRole.toLowerCase()}/measurement/${item._id}`} style={linkStyle}>
                        Update
                      </Link>
                    </TableCell>
                  </TableRow>
